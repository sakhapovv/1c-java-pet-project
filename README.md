...
после изменения статуса заказа в 1С система сразу отправляет новый статус в Java через endpoint: POST /api/orders/status
Если приложение временно недоступно, 1С не теряет изменение статуса заказа и автоматически доставляет его позже через очередь и регламентное задание.
